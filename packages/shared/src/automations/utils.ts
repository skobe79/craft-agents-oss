...
/**
 * Build the base ARCH_* environment variables shared by both prompt and webhook actions.
...
function buildBaseEventEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  const env: Record<string, string> = {
    ARCH_EVENT: event,
    ARCH_EVENT_DATA: JSON.stringify(payload),
  };

  if (payload.sessionId) env.ARCH_SESSION_ID = payload.sessionId;
  if (payload.sessionName) env.ARCH_SESSION_NAME = payload.sessionName;
  if (payload.workspaceId) env.ARCH_WORKSPACE_ID = payload.workspaceId;
  ...
  if (Object.keys(sessionMetadata).length > 0) {
    env.ARCH_SESSION_METADATA = JSON.stringify(sessionMetadata);
  }

  // Local time for scheduler events
  if (event === 'SchedulerTick') {
    const now = new Date();
    env.ARCH_LOCAL_TIME = now.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
    env.ARCH_LOCAL_DATE = now.toISOString().split('T')[0]!;
  }

  // Payload fields as ARCH_ vars (raw — callers apply sanitization if needed)
  for (const [key, value] of Object.entries(payload)) {
    if (PAYLOAD_SKIP_KEYS.has(key)) continue;
    const envKey = `ARCH_${toSnakeCase(key).toUpperCase()}`;
...
export function buildEnvFromPayload(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  if (payload.sessionName) env.ARCH_SESSION_NAME = sanitizeForShell(payload.sessionName);
 ...
 * - Only injects ARCH_WH_* user-defined vars from process.env (webhook secrets)
 * - Includes ARCH_* system vars derived from the event payload
 *
 * Users set webhook secrets in their shell profile:
 *   export ARCH_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
 *   export ARCH_WH_DISCORD_TOKEN="abc123"
 *
 * Then reference them in automations.json:
 *   "url": "${ARCH_WH_SLACK_URL}"
 *   "headers": { "Authorization": "Bearer ${ARCH_WH_DISCORD_TOKEN}" }
 */
export function buildWebhookEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  // User-defined webhook secrets: only ARCH_WH_* from process.env
  for (const [key, value] of Object.entries(process.env)) {
    if (key.startsWith('ARCH_WH_') && value !== undefined) {
      env[key] = value;
    }
  }

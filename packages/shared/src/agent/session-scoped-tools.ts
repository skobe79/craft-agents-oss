...
- \`hooks\`: Validates {workspace}/hooks.json (event hooks configuration)
...
4. Once valid, changes take effect on next reload`,
    {
      target: z.enum(['config', 'sources', 'statuses', 'preferences', 'permissions', 'hooks', 'tool-icons', 'all']).describe(
      ...
      try {
        ...
        switch (args.target) {
          ...
          case 'permissions':
            ...
            break;
          case 'hooks':
            result = validateHooks(workspaceRootPath);
            break;
          case 'tool-icons':

...
beforeAll(() => {
  origFlag = process.env.ARCH_FEATURE_ARCH_AGENTS_CLI
...
afterAll(() => {
  if (origFlag === undefined) {
    ...
    process.env.ARCH_FEATURE_ARCH_AGENTS_CLI = origFlag
  }
})
...
describe('PiAgent pre-tool labels guard', () => {
  it('blocks Read on workspace labels/config.json even when session workingDirectory is outside workspace root', async () => {
    ...
    expect(String(response?.reason ?? '')).toContain('arch-agentz label --help')

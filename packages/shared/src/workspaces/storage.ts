 ...
 * Default location: ~/.arch-agentz/workspaces/
 */
...
const CONFIG_DIR = join(homedir(), '.arch-agentz');
...
/**
 * Get the default workspaces directory (~/.arch-agentz/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.arch-agentz/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.arch-agentz/workspaces/
 */
 ...
 * Resolution order: ENV (ARCH_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.ARCH_LOCAL_MCP_ENABLED;

...
describe('buildOAuthDeeplinkUrl', () => {
  it('returns deeplink URL when sessionId and deeplinkScheme are provided', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('archagentz://allSessions/session/260209-swift-river');
  });
  ...
  it('handles session ID with special characters (hyphens and numbers)', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('archagentz://allSessions/session/260209-swift-river-42');
  });
  ...
  it('handles session ID with URL-unsafe characters', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('archagentz://allSessions/session/session/with spaces&special=chars');
  });

...
There are two MCP source connection models depending on the backend:

**Claude / Pi — McpClientPool (centralized, main process)**
- `McpClientPool` (`src/mcp/mcp-pool.ts`) manages all MCP source connections in the Electron main process
- Pool syncs via `pool.sync(mcpServers)` — connects new sources, disconnects removed ones
...
- API sources (REST APIs) are handled separately — they use in-process `createSdkMcpServer()` instances
  - Claude stores them in `sourceApiServers` and spreads into `fullMcpServers`
  - Pi routes them through the bridge-mcp-server subprocess

**Codex / Copilot — bridge-mcp-server + config.toml / buildMcpConfig()**
- These backends manage their own MCP connections via their SDK/binary
- Codex: MCP servers are configured in `config.toml` (generated by `config-generator.ts`)
- Copilot: MCP servers are passed via `buildMcpConfig()` at session creation
- API sources use the Bridge MCP Server subprocess (see below)
- The McpClientPool is NOT created for these backends

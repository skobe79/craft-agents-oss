...
const TEST_MODE_CONFIG = {
  ...
  readOnlyBashPatterns: [
    ...
    // arch-agentz CLI read-only
    { regex: /^arch-agentz\s+label\s+(list|get)\b/, source: '^arch-agentz\\s+label\\s+(list|get)\\b', comment: 'arch-agentz label read-only operations' },
    { regex: /^arch-agentz\s+source\s+(list|get|validate|test)\b/, source: '^arch-agentz\\s+source\\s+(list|get|validate|test)\\b', comment: 'arch-agentz source read-only operations' },
    { regex: /^arch-agentz\s+skill\s+(list|get|validate|where)\b/, source: '^arch-agentz\\s+skill\\s+(list|get|validate|where)\\b', comment: 'arch-agentz skill read-only operations' },
    { regex: /^arch-agentz\s+automation\s+(list|get|validate|history|last-executed|test|lint)\b/, source: '^arch-agentz\\s+automation\\s+(list|get|validate|history|last-executed|test|lint)\\b', comment: 'arch-agentz automation read-only operations' },
    { regex: /^arch-agentz\s*$/, source: '^arch-agentz\\s*$', comment: 'arch-agentz bare invocation' },
    { regex: /^arch-agentz\s+(label|source|skill|automation)\s+--help\b/, source: '^arch-agentz\\s+(label|source|skill|automation)\\s+--help\\b', comment: 'arch-agentz entity help flags' },
    { regex: /^arch-agentz\s+--(help|version|discover)\b/, source: '^arch-agentz\\s+--(help|version|discover)\\b', comment: 'arch-agentz global flags' },
...
// ============================================================
// Group 27: arch-agentz CLI allowlist
// ============================================================

describe('ShellGuard corpus: arch-agentz CLI allowlist', () => {
  const shouldAllow = [
    'arch-agentz',
    'arch-agentz --help',
    'arch-agentz --version',
    'arch-agentz --discover',
    'arch-agentz label --help',
    ...
    'arch-agentz skill get commit-helper',
    'arch-agentz skill where commit-helper',
    'arch-agentz skill validate commit-helper',
    'arch-agentz automation list',
    'arch-agentz automation get abc123',
    'arch-agentz automation validate',
    'arch-agentz automation history abc123 --limit 5',
    'arch-agentz automation last-executed abc123',
    'arch-agentz automation test abc123 --match "x"',
    'arch-agentz automation lint',
  ];

  const shouldBlock = [
    'arch-agentz label create --name Bug',
    'arch-agentz label update bug --name "Bug Report"',
    'arch-agentz label delete bug',
    'arch-agentz label move bug --parent root',
    'arch-agentz label reorder --parent root a b c',
    'arch-agentz source create --name Linear --provider linear --type mcp',
    'arch-agentz source update linear --json "{\"enabled\":false}"',
    'arch-agentz source delete linear',
    'arch-agentz skill create --name "Review" --description "x"',
    'arch-agentz skill update review --json "{\"description\":\"y\"}"',
    'arch-agentz skill delete review',
    'arch-agentz automation create --event UserPromptSubmit --prompt "x"',
    'arch-agentz automation update abc123 --json "{\"enabled\":false}"',
    'arch-agentz automation delete abc123',
    'arch-agentz automation enable abc123',
    'arch-agentz automation disable abc123',

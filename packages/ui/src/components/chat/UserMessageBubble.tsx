...
import type { StoredAttachment, ContentBadge } from '@arch-agentz/core'
import { normalizePath } from '@arch-agentz/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .arch-agentz workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.arch-agentz\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')

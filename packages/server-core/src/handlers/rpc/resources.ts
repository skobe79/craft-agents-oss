...
import { getCredentialManager, SOURCE_CREDENTIAL_TYPES } from '@arch-agentz/shared/credentials'
...
} from '@arch-agentz/shared/resources'
...
export function registerResourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, options: ExportResourcesOptions) => {
      ...
      const { exportResources } = await import('@arch-agentz/shared/resources')
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, bundle: ResourceBundle, mode: ResourceImportMode) => {
      ...
      const { importResources } = await import('@arch-agentz/shared/resources')

import { RPC_CHANNELS, type LlmConnectionSetup } from '@arch-agentz/shared/protocol'
import { getLlmConnections, getLlmConnection, addLlmConnection, updateLlmConnection, deleteLlmConnection, getDefaultLlmConnection, setDefaultLlmConnection, touchLlmConnection, isCompatProvider, isAnthropicProvider, getDefaultModelsForConnection, getDefaultModelForConnection, type LlmConnection, type LlmConnectionWithStatus, toBedrockNativeId, deriveBedrockRegionPrefix } from '@arch-agentz/shared/config'
import { getCredentialManager } from '@arch-agentz/shared/credentials'
import { setSetupDeferred } from '@arch-agentz/shared/config/storage'
import {
  resolveSetupTestConnectionHint,
  testBackendConnection,
  validateStoredBackendConnection,
} from '@arch-agentz/shared/agent/backend'
import { getModelRefreshService } from '@arch-agentz/server-core/model-fetchers'
import { parseTestConnectionError, createBuiltInConnection, validateModelList, piAuthProviderDisplayName, validateSetupTestInput, setupTestRequiresApiKey, resolveCustomEndpointSetup } from '@arch-agentz/server-core/domain'
import { getWorkspaceOrThrow, buildBackendHostRuntimeContext } from '@arch-agentz/server-core/handlers'
import { pushTyped, type RpcServer } from '@arch-agentz/server-core/transport'
import type { HandlerDeps } from '../handler-deps'
import { randomUUID } from 'node:crypto'
import { CLIENT_OPEN_EXTERNAL } from '@arch-agentz/server-core/transport'
...
export function registerLlmConnectionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.settings.SETUP_LLM_CONNECTION, async (_ctx, setup: LlmConnectionSetup): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (setup.piAuthProvider && !isCustomEndpointCompat) {
        ...
        // Update connection name to show the actual provider (e.g. "ARCH Agentz OS Backend (Google AI Studio)")
        const providerName = piAuthProviderDisplayName(setup.piAuthProvider)
        if (providerName) {
          updates.name = `ARCH Agentz OS Backend (${providerName})`
        }
  ...
  server.handle(RPC_CHANNELS.settings.TEST_LLM_CONNECTION_SETUP, async (_ctx, params: import('@arch-agentz/shared/protocol').TestLlmConnectionParams): Promise<import('@arch-agentz/shared/protocol').TestLlmConnectionResult> => {
  ...
  server.handle(RPC_CHANNELS.pi.GET_API_KEY_PROVIDERS, async () => {
    const { getPiApiKeyProviders } = await import('@arch-agentz/shared/config')
    return getPiApiKeyProviders()
  })

  server.handle(RPC_CHANNELS.pi.GET_PROVIDER_BASE_URL, async (_ctx, provider: string) => {
    const { getPiProviderBaseUrl } = await import('@arch-agentz/shared/config')
  ...
  server.handle(RPC_CHANNELS.llmConnections.TEST, async (_ctx, slug: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      const { parseValidationError } = await import('@arch-agentz/shared/config')
  ...
  server.handle(RPC_CHANNELS.chatgpt.START_OAUTH, async (ctx, connectionSlug: string): Promise<{
    ...
    const { prepareChatGptOAuth } = await import('@arch-agentz/shared/auth')
  ...
  server.handle(RPC_CHANNELS.chatgpt.COMPLETE_OAUTH, async (ctx, args: {
    ...
    try {
      const { exchangeChatGptTokens } = await import('@arch-agentz/shared/auth')
      const credentialManager = getCredentialManager()

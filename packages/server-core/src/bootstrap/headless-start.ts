...
import { ensureConfigDir, loadStoredConfig, saveConfig } from '@arch-agentz/shared/config'
import { CONFIG_DIR } from '@arch-agentz/shared/config/paths'
        ...
        if (lock.pid === process.pid) {
          ...
          if (isLockFromPreviousBoot(lock.startedAt)) {
            ...
            throw new Error(
              ...
              `To run a parallel instance (e.g. for dev), set ARCH_CONFIG_DIR to a different path.`
            )
          }
        } else {
...
export async function bootstrapServer<TSessionManager, THandlerDeps>(
  ...
  const serverToken = options.serverToken ?? process.env.ARCH_SERVER_TOKEN
  if (!serverToken) {
    throw new Error('Server token is required. Pass options.serverToken or set ARCH_SERVER_TOKEN.')
  }
  ...
  const bundledAssetsRoot = options.bundledAssetsRoot
    ?? process.env.ARCH_BUNDLED_ASSETS_ROOT
    ?? process.cwd()
  ...
  const rpcHost = options.rpcHost ?? process.env.ARCH_RPC_HOST ?? '127.0.0.1'
  const rpcPortRaw = options.rpcPort ?? parseInt(process.env.ARCH_RPC_PORT ?? '9100', 10)

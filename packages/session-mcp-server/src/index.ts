 ...
 * It uses the shared handlers from @arch-agentz/session-tools-core to ensure
 ...
 *   --workspace-root: Path to workspace folder (~/.arch-agentz/workspaces/{id})
...
import { isDeveloperFeedbackEnabled } from '@arch-agentz/shared/feature-flags';
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.arch-agentz/workspaces/{id}
      // preferencesPath = ~/.arch-agentz/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.ARCH_CONFIG_DIR || join(workspaceRootPath, '..', '..');
...
// ============================================================
// ARCH Agentz OS Docs Upstream Proxy
// ============================================================
...
/**
 * Connect to the arch-agentzs-docs MCP server and fetch its tool definitions.
...
async function connectDocsUpstream(): Promise<void> {
  try {
    const client = new Client(
      { name: 'arch-agentz-session-proxy', version: '1.0.0' },
      { capabilities: {} }
    );
    ...
    console.error(`ARCH Agentz OS Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`ARCH Agentz OS Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`ARCH Agentz OS Docs server is not connected. Tool '${name}' unavailable.`);
  }
...
async function handleCallLlm(
  ...
  // Uses callbackPort from CLI arg (--callback-port) or env var (ARCH_LLM_CALLBACK_PORT).
  ...
  return errorResponse(
    ...
    'HTTP callback (ARCH_LLM_CALLBACK_PORT). Neither is available.'
  );
}
...
async function main() {
  ...
  const config: SessionConfig = {
    ...
    callbackPort: callbackPort || process.env.ARCH_LLM_CALLBACK_PORT,
  };

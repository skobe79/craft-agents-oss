...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to ARCH_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.ARCH_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.ARCH_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.ARCH_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute ARCH_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute ARCH_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (ARCH_UV / ARCH_NODE / ARCH_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.ARCH_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ARCH_UV, 'Python/uv')
        : process.env.ARCH_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure ARCH_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.ARCH_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ARCH_NODE, 'Node')
        : process.env.ARCH_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure ARCH_NODE or install node on PATH.'
    );
  }

  if (process.env.ARCH_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.ARCH_BUN, 'Bun')
      : process.env.ARCH_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure ARCH_BUN or install bun on PATH.'
  );
}

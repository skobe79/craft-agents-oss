...
describe('parseMermaid – sourceOrder', () => {
  it('tracks top-level nodes in source order', () => {
    const g = parseMermaid('graph TD\n  A --> B --> C')
    expect(g.sourceOrder).toEqual([
      { type: 'node', id: 'A' },
      { type: 'node', id: 'B' },
      { type: 'node', id: 'C' },
    ])
  ...
  it('tracks subgraph before top-level nodes when subgraph is defined first', () => {
    const g = parseMermaid(`graph TD
      subgraph ci [CI Pipeline]
        A[Push Code] --> B{Tests Pass?}
      end
      C --> D`)
    // Subgraph ci completes at "end" before C and D are encountered
    expect(g.sourceOrder).toEqual([
      { type: 'subgraph', id: 'ci' },
      { type: 'node', id: 'C' },
      { type: 'node', id: 'D' },
  ...
  it('tracks top-level nodes before subgraph when nodes are defined first', () => {
    const g = parseMermaid(`graph TD
      A --> B
      subgraph sg [Group]
        C --> D
  ...
  it('does not include subgraph member nodes in sourceOrder', () => {
    const g = parseMermaid(`graph TD
      subgraph sg [Group]
        X --> Y
      end
      Z`)
    // X and Y are inside the subgraph, so they should NOT appear in sourceOrder
    const nodeIds = g.sourceOrder.filter(e => e.type === 'node').map(e => e.id)
    expect(nodeIds).not.toContain('X')
    expect(nodeIds).not.toContain('Y')
    expect(nodeIds).toContain('Z')
  })

  it('only tracks root-level subgraphs (not nested children)', () => {
    const g = parseMermaid(`graph TD
      ...
      C`)
    // Only the outer subgraph should appear in sourceOrder
    const sgIds = g.sourceOrder.filter(e => e.type === 'subgraph').map(e => e.id)
    expect(sgIds).toEqual(['outer'])
  })

  it('tracks state diagram sourceOrder with composite states', () => {
    const g = parseMermaid(`stateDiagram-v2
      ...
      Complete --> [*]`)
    // _start and Idle are top-level nodes, Processing is a composite state (subgraph),
    // Complete and _end are top-level nodes
    expect(g.sourceOrder).toEqual([
      { type: 'node', id: '_start' },
      { type: 'node', id: 'Idle' },
      { type: 'node', id: 'Processing' },
      { type: 'subgraph', id: 'Processing' },
      { type: 'node', id: 'Complete' },
      { type: 'node', id: '_end' },
    ])
  })
})

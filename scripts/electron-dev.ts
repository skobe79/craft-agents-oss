...
// Detects instance number from folder name suffix (e.g., arch-agentzs-1 → instance 1)
function detectInstance(): void {
  // Don't override if already set (e.g., by sourcing detect-instance.sh first)
  if (process.env.ARCH_VITE_PORT) return;
  ...
  if (match) {
    ...
    process.env.ARCH_INSTANCE_NUMBER = instanceNum;
    process.env.ARCH_VITE_PORT = `${instanceNum}173`;
    process.env.ARCH_APP_NAME = `ARCH Agentz OS [${instanceNum}]`;
    process.env.ARCH_CONFIG_DIR = join(process.env.HOME || "", `.arch-agentz-${instanceNum}`);
    process.env.ARCH_DEEPLINK_SCHEME = `craftagents${instanceNum}`;
    console.log(`🔢 Instance ${instanceNum} detected: port=${process.env.ARCH_VITE_PORT}, config=${process.env.ARCH_CONFIG_DIR}`);
  }
}
...
function getElectronEnv(): Record<string, string> {
  const vitePort = process.env.ARCH_VITE_PORT || "5173";
  ...
  return {
    ...
    ARCH_CONFIG_DIR: process.env.ARCH_CONFIG_DIR || "",
    ARCH_APP_NAME: process.env.ARCH_APP_NAME || "ARCH Agentz OS",
    ARCH_DEEPLINK_SCHEME: process.env.ARCH_DEEPLINK_SCHEME || "craftagents",
    ARCH_INSTANCE_NUMBER: process.env.ARCH_INSTANCE_NUMBER || "",
  };
}

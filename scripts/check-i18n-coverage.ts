...
export function walkSourceTree(dir: string, out: string[] = []): string[] {
  let entries: string[]
  try {
    entries = readdirSync(dir)
  } catch (error) {
    throw new Error(`Unable to read configured i18n source tree: ${dir}`, { cause: error })
  }
  for (const name of entries) {
    ...
    try {
      ...
      throw new Error(`Unable to inspect i18n source path: ${full}`, { cause: error })
    }
    if (st.isDirectory()) {
...
function main(): void {
  ...
  for (const dir of SCAN_DIRS) {
    walkSourceTree(join(ROOT, dir), files)
  }

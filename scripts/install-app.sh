...
DOWNLOAD_DIR="$HOME/.arch-agentz/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.lukilabs.arch-agentz"
    if pgrep -x "ARCH Agentz OS" >/dev/null 2>&1; then
        info "Quitting ARCH Agentz OS..."
        ...
        if pgrep -x "ARCH Agentz OS" >/dev/null 2>&1; then
            ...
            pkill -9 -x "ARCH Agentz OS" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  ARCH Agentz OS has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'ARCH Agentz OS'${NC}"
    echo ""
...
APPIMAGE_PATH="$HOME/.arch-agentz/app/Craft-Agents-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@arch-agentz"
ELECTRON_CACHE_ALT="$HOME/.cache/@arch-agentz"

...
<Note>
The `anthropic_api_key` credential type is used for all API key-based providers, not just Anthropic. The `anthropicBaseUrl` field in `config.json` determines which provider the key is used with. See [API Providers](/reference/config/custom-endpoint) for details.
</Note>

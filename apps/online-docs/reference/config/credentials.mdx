...
```
{type}::{workspaceId}::{sourceSlug}
```

Where `type` identifies the credential type, `workspaceId` is the workspace UUID, and `sourceSlug` is the source identifier.
...
Source (3-part key: type::workspaceId::sourceSlug)
  source_oauth::ws-abc123::github
...
<AccordionGroup>
  <Accordion title="Credential not found errors">
    ...
    - For API keys: `/settings` and re-enter key

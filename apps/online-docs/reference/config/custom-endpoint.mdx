...
- The **base URL** as `anthropicBaseUrl` in `config.json`
- The **model override** as `customModel` in `config.json`
...
The base URL and model name are stored in `config.json` (not encrypted, as they are not sensitive).

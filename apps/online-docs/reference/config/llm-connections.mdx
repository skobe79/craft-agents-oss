...
When using `iam_credentials`, enter your AWS Access Key ID, Secret Access Key, and (optionally) Session Token in the UI. Credentials are encrypted and stored per connection. The subprocess receives them as scoped environment variables — they are **not** written to `~/.aws/credentials`.

<Note>
If you use AWS STS temporary credentials (session tokens), they expire after ~1 hour. You'll need to update them when they expire.
</Note>

### AWS Bedrock (Environment Auth)

```json
{
  "slug": "bedrock-env",
  "name": "AWS Bedrock (Environment)",
  "providerType": "bedrock",
...
With `environment` auth, the subprocess inherits your shell's AWS credential chain — `~/.aws/credentials`, `AWS_PROFILE`, IAM roles, SSO sessions, etc. No credentials are stored in Craft Agents.

...
| `mcp.transport` | No | Transport type: `"http"` (default) for remote servers, `"stdio"` for local servers |
| `mcp.url` | Yes (HTTP) | Full URL to the MCP server endpoint (for HTTP transport) |
...
| `mcp.authType` | Yes (HTTP) | Authentication type: `"oauth"`, `"bearer"`, or `"none"` |

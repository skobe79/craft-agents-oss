...
export function parseArgs(argv: string[]): CliArgs {
  ...
  // Env var fallbacks
  if (!url) url = process.env.ARCH_SERVER_URL ?? ''
  if (!token) token = process.env.ARCH_SERVER_TOKEN ?? ''
  if (!tlsCa) tlsCa = process.env.ARCH_TLS_CA
...
Connection:
  --url <ws[s]://...>    Server URL (default: $ARCH_SERVER_URL)
  --token <secret>       Auth token (default: $ARCH_SERVER_TOKEN)
...
export async function main(argv: string[] = process.argv): Promise<void> {
  ...
  if (!args.url) {
    err('No server URL. Use --url <ws://...> or set $ARCH_SERVER_URL')
    process.exit(1)
  }

...
import type { RpcServer, RequestContext } from '@arch-agentz/server-core/transport'
...
describe('session file watcher isolation', () => {
  ...
  it('creates independent watchers per client and cleans up on disconnect', async () => {
    ...
    const { registerSessionsHandlers, cleanupSessionFileWatchForClient } = await import('@arch-agentz/server-core/handlers/rpc')

...
import { APP_EVENTS, AGENT_EVENTS, type AutomationFilterKind, AUTOMATION_TYPE_TO_FILTER_KIND } from "../components/automations/types"
import { useAutomations } from "@/hooks/useAutomations"
import { useProjects } from "@/hooks/useProjects"
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from "@/components/ui/dialog"
import { PanelHeader } from "../components/app-shell/PanelHeader"
import { FabNewChat } from "../components/app-shell/FabNewChat"
import { SendToWorkspaceDialog } from "../components/app-shell/SendToWorkspaceDialog"
...
/**
 * ChatSessionAreaProps - Minimal props interface for ChatSessionArea component
 *
 ...
 */
export interface ChatSessionAreaProps {
 ...
 */
export function ChatSessionArea(props: ChatSessionAreaProps) {
  // Wrap with EscapeInterruptProvider so AppShellContent can use useEscapeInterrupt
  return (
    <EscapeInterruptProvider>
      <ChatSessionAreaContent {...props} />
    </EscapeInterruptProvider>
  )
}
 ...
 */
function ChatSessionAreaContent({
  contextValue,
  defaultLayout = [20, 32, 48],
  defaultCollapsed = false,
  menuNewChatTrigger,
  isFocusedMode = false,
}: ChatSessionAreaProps) {
  ...
  // Collapse state for the unified ARCHstudio brand rail (Ctrl+B). The rail now
  // hosts the session list + LeftSidebar; collapsing hides the whole rail.
  const [archSidebarCollapsed, setArchSidebarCollapsed] = React.useState(() => {
    return storage.get(storage.KEYS.archSidebarCollapsed, false)
  })
  ...
  // Persist brand-rail collapse state to localStorage
  React.useEffect(() => {
    storage.set(storage.KEYS.archSidebarCollapsed, archSidebarCollapsed)
  }, [archSidebarCollapsed])

...
- Fixed auto-activation failing for sources with `authType: none` (e.g., Context7) ([#72](https://github.com/lukilabs/arch-agentzs-oss/issues/72))
- `source_test` now correctly sets `isAuthenticated` for MCP, API, and local sources that don't require authentication
- Fixed session date grouping changing after app restart by persisting `lastMessageAt` separately ([#69](https://github.com/lukilabs/arch-agentzs-oss/issues/69))
- Fixed empty placeholder docs by copying documentation assets into Electron dist ([#70](https://github.com/lukilabs/arch-agentzs-oss/issues/70))
- Fixed OSS `electron:dist:mac` builds by adding missing scripts to allow-list ([#71](https://github.com/lukilabs/arch-agentzs-oss/issues/71))

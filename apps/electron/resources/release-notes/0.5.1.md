...
- **Skill metadata** — `requiredSources` and `icon` fields are now displayed in the skill info page ([#290](https://github.com/lukilabs/arch-agentzs-oss/issues/290))
...
- **File mentions** — File and folder mentions in chat messages are no longer stripped; they now resolve to absolute paths for the agent ([#293](https://github.com/lukilabs/arch-agentzs-oss/issues/293))
...
- **Install scripts** — Fetch YAML manifests directly instead of relying on the version endpoint; multiple PowerShell 5.1 compatibility fixes ([#273](https://github.com/lukilabs/arch-agentzs-oss/issues/273))

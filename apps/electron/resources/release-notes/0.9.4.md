...
- **Skills “Show in Finder” opens the real skill folder** — Skills list, skill detail, and skill menu actions now reveal the authoritative `skill.path` directory instead of constructing a synthetic `SKILL.md` path, and failures surface via a platform-aware Finder/Explorer toast instead of silently no-oping. Fixes [#756](https://github.com/lukilabs/arch-agentzs-oss/issues/756). (`d44e5691`)

...
### Why it's cool
This is the first step toward full programmatic control of ARCH Agentz OS. Script your agent workflows, integrate into CI/CD pipelines, or run sessions headlessly on remote machines. Partially addresses [#343](https://github.com/lukilabs/arch-agentzs-oss/issues/343).

## Headless Server

Run ARCH Agentz OS as a standalone server without the Electron desktop app — deploy on any machine, connect from any client.
...
### Why it's cool
Deploy ARCH Agentz OS on a remote server and connect from anywhere. Run it on your home lab, a cloud VM, or a CI runner. The thin-client mode means the browser UI stays lightweight while all the heavy lifting happens server-side. Partially addresses [#291](https://github.com/lukilabs/arch-agentzs-oss/issues/291).
...
- **Server-core extraction** — 14 core handlers, session management, services, and domain utilities extracted into `@arch-agentz/server-core` for reuse across Electron and headless
- **Protocol formalization** — channels, DTOs, and event maps extracted to `@arch-agentz/shared/protocol` with wire-format stability tests
...
- **Pi 3-tier model persistence** — fixed model save/hydration for Best/Balanced/Fast tiers. Fixes [#336](https://github.com/lukilabs/arch-agentzs-oss/issues/336)
- **Multi-panel input focus** — fixed focus scoping and race handling that caused keystrokes to jump between panels. Fixes [#339](https://github.com/lukilabs/arch-agentzs-oss/issues/339)

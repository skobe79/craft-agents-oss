...
> **CLI-first workflow (recommended):** Use `arch-agentz automation ...` commands instead of editing JSON directly.
> - `arch-agentz automation --help`
...
```
~/.arch-agentz/workspaces/{workspaceId}/automations.json
```
...
arch-agentz automation create --event UserPromptSubmit --prompt "..."
arch-agentz automation update <id> --json '{...}'
...
- Environment variables are expanded (e.g., `$ARCH_LABEL`)
...
{
  "type": "webhook",
  "url": "https://hooks.slack.com/services/${ARCH_WH_SLACK_PATH}",
  "method": "POST",
  "body": {
    "text": "Session ${ARCH_SESSION_NAME} status changed to ${ARCH_NEW_STATE}"
  }
}
```
...
```json
{
  ...
  "auth": {
    "type": "bearer",
    "token": "${ARCH_WH_API_TOKEN}"
  },
  "body": { "event": "$ARCH_EVENT" }
}
```
...
```json
{
  ...
  "auth": {
    "type": "basic",
    "username": "${ARCH_WH_USER}",
    "password": "${ARCH_WH_PASS}"
  }
}
```
...
**Security:** Webhook actions only have access to `ARCH_*` system variables and `ARCH_WH_*` user-defined secrets. They do **not** have access to your full system environment (e.g., `$HOME`, `$PATH`, or other process variables).

## Environment Variables

Both prompt and webhook actions support variable expansion using `$VAR` or `${VAR}` syntax.

### System Variables (ARCH_*)

These are automatically set by the automation system based on the triggering event:

| Variable | Description | Available For |
|----------|-------------|---------------|
| `$ARCH_EVENT` | Event name (e.g., `LabelAdd`) | All events |
| `$ARCH_EVENT_DATA` | Full event payload as JSON | All events |
| `$ARCH_SESSION_ID` | Session ID | Events with session context |
| `$ARCH_SESSION_NAME` | Session name | Events with session context |
| `$ARCH_WORKSPACE_ID` | Workspace ID | All events |

**Per-event variables:**

| Event | Variable | Description |
|-------|----------|-------------|
| `LabelAdd` / `LabelRemove` | `$ARCH_LABEL` | The label that was added/removed |
| `PermissionModeChange` | `$ARCH_OLD_MODE`, `$ARCH_NEW_MODE` | Previous and new permission mode |
| `FlagChange` | `$ARCH_IS_FLAGGED` | `true` or `false` |
| `SessionStatusChange` | `$ARCH_OLD_STATE`, `$ARCH_NEW_STATE` | Previous and new status |
| `SchedulerTick` | `$ARCH_LOCAL_TIME`, `$ARCH_LOCAL_DATE` | Current time (`14:30`) and date (`2026-03-09`) |

### User-Defined Webhook Secrets (ARCH_WH_*)

For webhook actions, you can define your own secrets by setting environment variables with the `ARCH_WH_` prefix in your shell profile (e.g., `~/.zshrc`, `~/.bashrc`):

```bash
# In your shell profile
export ARCH_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
export ARCH_WH_DISCORD_URL="https://discord.com/api/webhooks/123/abc"
export ARCH_WH_API_TOKEN="your-secret-token"
```
...
```json
{
  "type": "webhook",
  "url": "https://api.example.com/events",
  "headers": { "Authorization": "Bearer ${ARCH_WH_API_TOKEN}" },
  "body": { "event": "${ARCH_EVENT}", "session": "${ARCH_SESSION_NAME}" }
}
```

This keeps secrets out of `automations.json` (which may be shared or committed to version control).

> **Note:** Only variables prefixed with `ARCH_WH_` are injected into webhook actions. Other environment variables (like `$HOME` or `$DATABASE_URL`) are not accessible to webhooks.
...
Verify by checking the supergroup row in Settings shows the group title. If automation runs fail later, `~/.arch-agentz/logs/messaging-gateway.log` will show `automation_topic_bind_failed` with the underlying Telegram error.
        ...
        "actions": [
          {
            ...
            "url": "${ARCH_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":warning: Permission escalated from safe to allow-all in *${ARCH_SESSION_NAME}*" }
          }
        ]
      }
    ]
...
```json
{
  ...
  "automations": {
    "LabelAdd": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $ARCH_LABEL was added. Log this change with a timestamp." }
        ]
      }
    ],
    "LabelRemove": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $ARCH_LABEL was removed. Log this change with a timestamp." }
        ]
      }
    ]
  }
...
Sends a Slack message when a session is marked as done. Requires `ARCH_WH_SLACK_URL` in your shell profile.
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "body": {
              "text": ":white_check_mark: Session *${ARCH_SESSION_NAME}* marked as done"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "url": "${ARCH_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":rotating_light: Urgent label added to *${ARCH_SESSION_NAME}*" }
          },
          {
            "type": "prompt",
        ...
        "actions": [
          {
            ...
            "body": {
              ...
              "client_id": "${ARCH_WH_CLIENT_ID}",
              "client_secret": "${ARCH_WH_CLIENT_SECRET}"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "headers": {
              "Authorization": "Bearer ${ARCH_WH_API_TOKEN}",
              "X-Source": "arch-agentz"
            },
            "body": {
              "event": "${ARCH_EVENT}",
              "session_id": "${ARCH_SESSION_ID}",
              "old_status": "${ARCH_OLD_STATE}",
              "new_status": "${ARCH_NEW_STATE}"
            }
          }
        ]
      }
...
2. **Check env vars** — Ensure `ARCH_WH_*` variables are set in your shell profile and Craft Agent was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
...
5. **Keep secrets out of config** - Use `ARCH_WH_*` env vars for webhook URLs and tokens instead of hardcoding them in automations.json
